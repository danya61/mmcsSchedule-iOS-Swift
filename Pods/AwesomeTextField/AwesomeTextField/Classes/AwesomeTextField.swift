...
class AwesomeTextField: UITextField {
    ...
    @IBInspectable var underLineWidth : CGFloat = 1.9
    ...
    func drawLine() {
        let underLine = UIView(frame:CGRect(x: 0,
                                            y: self.frame.size.height - self.underLineWidth,
                                            width: self.frame.size.width,
                                            height: self.underLineWidth))
    ...
    override func drawPlaceholder(in rect: CGRect) {
        ...
        self.placeholderLabel = UILabel(frame: CGRect(x: rect.origin.x,
                                                      y: self.underLineWidth,
                                                      width: rect.size.width,
                                                      height: self.font!.pointSize - underLineWidth))
        self.placeholderLabel.center = CGPoint(x: self.placeholderLabel.center.x,
                                               y: self.frame.size.height - self.underlineView.frame.size.height - self.placeholderLabel.frame.size.height / 2)
        self.placeholderLabel.text = self.placeholder
        self.placeholder = nil
        
        self.placeholderLabel.font = UIFont(name: self.font!.fontName, size: self.font!.pointSize - 2.8)
    ...
    func didBeginChangeText() {
        ...
        if !self.isLifted {
            UIView.animate(withDuration: self.animationDuration, animations: { () -> Void in
                self.placeholderLabel.transform = CGAffineTransform(scaleX: self.scaleCoeff, y: self.scaleCoeff)
                self.placeholderLabel.alpha = self.placeholderAlphaAfter
                self.placeholderLabel.center = CGPoint.init(x: self.placeholderLabel.center.x * self.scaleCoeff,
                                                            y: 0 + self.placeholderLabel.frame.size.height)
								self.placeholderLabel.font =  UIFont(name: self.font!.fontName, size: self.font!.pointSize - 3.0)
    ...
    func didEndChangeText() {
        ...
        if self.isLifted && self.text!.isEmpty {
            UIView.animate(withDuration: self.animationDuration, animations: { () -> Void in
                ...
                self.placeholderLabel.alpha = self.placeholderAlphaBefore
							self.placeholderLabel.center = CGPoint.init(x: self.placeholderLabel.center.x / self.scaleCoeff,
							                                            y: self.frame.size.height - self.underlineView.frame.size.height - self.placeholderLabel.frame.size.height / 2.0 - 2.0)
